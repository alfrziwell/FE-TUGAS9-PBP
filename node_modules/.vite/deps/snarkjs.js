...
//#region node_modules/ffjavascript/build/browser.esm.js
var hexLen = [
	0,
	1,
	2,
...
function toRprLE(buff, o, e, n8) {
	...
	for (let i = v.length * 4; i < n8; i++) buff[i] = toNumber$1(band(shiftRight(e, i * 8), 255));
}
...
var ChaCha = class {
	...
	nextU64() {
		return add$1(mul(this.nextU32(), 4294967296), this.nextU32());
	}
	nextBool() {
...
var ZqField = class {
	...
	random() {
		...
		let res = this.zero;
		for (let i = 0; i < nBytes; i++) res = (res << BigInt(8)) + BigInt(getRandomBytes$1(1)[0]);
		return res % this.p;
	}
	toString(a, base) {
		base = base || 10;
...
function bitLength$5(n) {
	if (isNegative$3(n)) return n.toString(2).length - 1;
...
var build_f1m = function buildF1m(module, _q, _prefix, _intPrefix) {
	...
	function buildMReduct() {
		...
		f.addLocal("m", "i64");
		const c = f.getCodeBuilder();
		const np32 = Number(4294967296n - modInv$2(q, 4294967296n));
	...
	let _nqr = 2n;
	if (isPrime(q)) while (modPow$1(_nqr, _e, q) !== _minusOne) _nqr = _nqr + 1n;
	let s2 = 0;
	let _t = _minusOne;
...
var buildTimesScalarNAF = build_timesscalarnaf;
...
var build_bn128 = function buildBN128(module, _prefix) {
	...
	const pG2b = module.alloc([...utils$2.bigInt2BytesLE(toMontgomery(19485874751759354771024239261021720505790618469301721065564631296452457478373n), f1size), ...utils$2.bigInt2BytesLE(toMontgomery(266929791119991161246907387137283842545076965332900288569378510910307636690n), f1size)]);
	...
	const pNonResidueF6 = module.alloc([...utils$2.bigInt2BytesLE(toMontgomery(9), f1size), ...utils$2.bigInt2BytesLE(toMontgomery(1), f1size)]);
	const pTwoInv = module.alloc([...utils$2.bigInt2BytesLE(toMontgomery(modInv(2n, q)), f1size), ...utils$2.bigInt2BytesLE(0n, f1size)]);
	const pAltBn128Twist = pNonResidueF6;
	const pTwistCoefB = module.alloc([...utils$2.bigInt2BytesLE(toMontgomery(19485874751759354771024239261021720505790618469301721065564631296452457478373n), f1size), ...utils$2.bigInt2BytesLE(toMontgomery(266929791119991161246907387137283842545076965332900288569378510910307636690n), f1size)]);
	...
	function buildMulByQ() {
		...
		const MulByQX = c.i32_const(module.alloc([...utils$2.bigInt2BytesLE(toMontgomery("21575463638280843010398324269430826099269044274347216827212613867836435027261"), f1size), ...utils$2.bigInt2BytesLE(toMontgomery("10307601595873709700152284273816112264069230130616436755625194854815875713954"), f1size)]));
		const MulByQY = c.i32_const(module.alloc([...utils$2.bigInt2BytesLE(toMontgomery("2821565182194536844548159561693502659359617185244120367078079554186484126554"), f1size), ...utils$2.bigInt2BytesLE(toMontgomery("3505843767911556378687030309984248845540243509899259641013678093033130930403"), f1size)]));
	...
	function buildFrobeniusMap(n) {
		...
		for (let i = 0; i < 6; i++) {
			...
			const pCoef = module.alloc([...utils$2.bigInt2BytesLE(toMontgomery(coef[0]), 32), ...utils$2.bigInt2BytesLE(toMontgomery(coef[1]), 32)]);
	...
	function buildFinalExponentiationOld() {
		...
		f.addParam("x", "i32");
		f.addParam("r", "i32");
		const pExponent = module.alloc(utils$2.bigInt2BytesLE(, 352));
		const c = f.getCodeBuilder();
...
var utils$1 = utils$6;
var buildF1m = build_f1m;
var buildF1 = build_f1;
var buildF2m = build_f2m;
var buildF3m = build_f3m;
var buildCurve = build_curve_jacobian_a0;
var buildFFT$1 = build_fft;
var buildPol = build_pol;
var buildQAP = build_qap;
var buildApplyKey = build_applykey;
...
var build_bls12381 = function buildBLS12381(module, _prefix) {
	...
	const pG2b = module.alloc([...utils$1.bigInt2BytesLE(toMontgomery(4n), f1size), ...utils$1.bigInt2BytesLE(toMontgomery(4n), f1size)]);
	...
	const pBls12381Twist = module.alloc([...utils$1.bigInt2BytesLE(toMontgomery(1n), f1size), ...utils$1.bigInt2BytesLE(toMontgomery(1n), f1size)]);
	...
	function buildFrobeniusMap(n) {
		...
		for (let i = 0; i < 6; i++) {
			...
			const Rc1 = c.i32_add(c.getLocal("r"), c.i32_const(i * f2size + f1size));
			const coef = mul2(F12[Math.floor(i / 3)][n % 12], F6[i % 3][n % 6]);
			const pCoef = module.alloc([...utils$1.bigInt2BytesLE(toMontgomery(coef[0]), n8q), ...utils$1.bigInt2BytesLE(toMontgomery(coef[1]), n8q)]);
			if (n % 2 == 1) f.addCode(c.call(f1mPrefix + "_copy", Xc0, Rc0), c.call(f1mPrefix + "_neg", Xc1, Rc1), c.call(f2mPrefix + "_mul", R, c.i32_const(pCoef), R));
			else f.addCode(c.call(f2mPrefix + "_mul", X, c.i32_const(pCoef), R));
		}
		function mul2(a, b) {
	...
	function buildFinalExponentiationOld() {
		...
		f.addParam("x", "i32");
		f.addParam("r", "i32");
		const pExponent = module.alloc(utils$1.bigInt2BytesLE(, 544));
		const c = f.getCodeBuilder();
	...
	function buildInGroupG2() {
		...
		const wInv = c.i32_const(module.alloc([...utils$1.bigInt2BytesLE(toMontgomery(WINV[0]), n8q), ...utils$1.bigInt2BytesLE(toMontgomery(WINV[1]), n8q)]));
		const frob2X = c.i32_const(module.alloc(utils$1.bigInt2BytesLE(toMontgomery(FROB2X), n8q)));
		const frob3Y = c.i32_const(module.alloc([...utils$1.bigInt2BytesLE(toMontgomery(FROB3Y[0]), n8q), ...utils$1.bigInt2BytesLE(toMontgomery(FROB3Y[1]), n8q)]));
...
function stringifyBigInts$5(o) {
	...
	else if (Array.isArray(o)) return o.map(stringifyBigInts$5);
...
function unstringifyBigInts$10(o) {
	if (typeof o == "string" && /^[0-9]+$/.test(o)) return BigInt(o);
	else if (typeof o == "string" && /^0x[0-9a-fA-F]+$/.test(o)) return BigInt(o);
	else if (Array.isArray(o)) return o.map(unstringifyBigInts$10);
	else if (typeof o == "object") {
		if (o === null) return null;
		const res = {};
		Object.keys(o).forEach((k) => {
			res[k] = unstringifyBigInts$10(o[k]);
		});
		return res;
	} else return o;
}
...
var _utils = /*#__PURE__*/ Object.freeze({
	...
	stringifyFElements,
	unstringifyBigInts: unstringifyBigInts$10,
...
var BigBuffer = class BigBuffer {
	...
	slice(fr, to) {
		...
		if (firstPage == Math.floor((fr + len - 1) / PAGE_SIZE$1) || len == 0) return this.buffers[firstPage].slice(fr % PAGE_SIZE$1, fr % PAGE_SIZE$1 + len);
		let buff;
		let p = firstPage;
		...
		while (r > 0) {
			...
			if (!buff) if (len <= PAGE_SIZE$1) buff = new Uint8Array(len);
	...
	set(buff, offset) {
		...
		if (firstPage == Math.floor((offset + len - 1) / PAGE_SIZE$1)) if (buff instanceof BigBuffer && buff.buffers.length == 1) return this.buffers[firstPage].set(buff.buffers[0], offset % PAGE_SIZE$1);
		else return this.buffers[firstPage].set(buff, offset % PAGE_SIZE$1);
		let p = firstPage;
			...
			const task = [
				{
					cmd: "ALLOCSET",
					var: 0,
					buff: buffIn.slice(i * pointsPerChunk * sIn, i * pointsPerChunk * sIn + n * sIn)
				},
				{
					cmd: "ALLOC",
					var: 1,
...
var WasmField1 = class {
	...
	fromRng(rng) {
		...
		do {
			v = zero;
			for (let i = 0; i < this.n64; i++) v = add$1(v, shiftLeft(rng.nextU64(), 64 * i));
			v = band(v, this.mask);
		} while (geq(v, this.p));
...
async function buildThreadManager(wasm, singleThread) {
	...
	if (!globalThis?.Worker) singleThread = true;
...
var ThreadManager = class {
	...
	async terminate() {
		for (let i = 0; i < this.workers.length; i++) this.workers[i].postMessage([{ cmd: "TERMINATE" }]);
		await sleep(200);
	}
};
...
function buildFFT(curve, groupName) {
	...
	async function _fftJoinExt(buff1, buff2, fn, first, inc, inType, outType, logger, loggerTxt) {
		...
		if (nPoints != 1 << log2$2(nPoints)) throw new Error("Invalid number of points");
...
function u32$1(n) {
	const b = [];
	const v = toNumber(n);
	b.push(Number(v & 255n));
	b.push(Number(v >> 8n & 255n));
	b.push(Number(v >> 16n & 255n));
	b.push(Number(v >> 24n & 255n));
	return b;
}
...
function varuint(n) {
	...
	while (!isZero(v)) {
		code.push(Number(v & 127n));
		v = v >> 7n;
	}
	if (code.length == 0) code.push(0);
	for (let i = 0; i < code.length - 1; i++) code[i] = code[i] | 128;
	return code;
}
function varint(_n) {
	let n, sign;
...
function varint32(n) {
	let v = toNumber(n);
	if (v > 4294967295n) throw new Error("Number too big");
	if (v > 2147483647n) v = v - 4294967296n;
	if (v < -2147483648n) throw new Error("Number too small");
	return varint(v);
}
function varint64(n) {
	let v = toNumber(n);
	if (v > 18446744073709551615n) throw new Error("Number too big");
	if (v > 9223372036854775807n) v = v - 18446744073709551616n;
...
var CodeBuilder = class {
	...
	setLocal(localName, valCode) {
		...
		if (idx === void 0) throw new Error(`Local Variable not defined: Function: ${this.functionName} local: ${localName} `);
		return [
			...valCode,
			33,
			...varuint32(idx)
	...
	call(fnName, ...args) {
		...
		if (idx === void 0) throw new Error(`Function not defined: Function: ${fnName}`);
		return [
			...[].concat(...args),
			16,
			...varuint32(idx)
...
var FunctionBuilder = class {
	...
	addParam(paramName, paramType) {
		...
		this.params.push({ type: paramType });
	}
...
var ModuleBuilder = class {
	...
	allocString(s) {
		const uint8array = new globalThis.TextEncoder().encode(s);
	...
	_buildType() {
		return this._buildSection(1, [...varuint32(this.signatures.length), ...[].concat(...this.signatures)]);
	}
	_buildImport() {
		const entries = [];
		entries.push([
		...
		return this._buildSection(2, varuint32(entries.length).concat(...entries));
	}
	_buildFunctionDeclarations() {
		const entries = [];
		for (let i = this.nImportFunctions; i < this.nImportFunctions + this.nInternalFunctions; i++) entries.push(...varuint32(this.functions[i].signatureIdx));
		return this._buildSection(3, [...varuint32(entries.length), ...[...entries]]);
	}
	...
	_buildElements() {
		if (this.functionsTable.length == 0) return [];
		const entries = [];
		for (let i = 0; i < this.functionsTable.length; i++) entries.push(...varuint32(this.functionsTable[i]));
	...
	_buildExports() {
		...
		return this._buildSection(7, varuint32(entries.length).concat(...entries));
	}
	_buildCode() {
		const entries = [];
		for (let i = this.nImportFunctions; i < this.nImportFunctions + this.nInternalFunctions; i++) entries.push(this.functions[i].getBody());
		return this._buildSection(10, varuint32(entries.length).concat(...entries));
	}
	_buildData() {
		const entries = [];
		entries.push([
		...
		return this._buildSection(11, varuint32(entries.length).concat(...entries));
	}
};
...
var bls12381r$2 = e("73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001", 16);
var bn128r$2 = e("21888242871839275222246405745257275088548364400416034343698204186575808495617");
...
async function getCurveFromR$1(r, singleThread, plugins) {
	let curve;
	if (eq(r, bn128r$2)) curve = await buildBn128(singleThread, plugins);
	else if (eq(r, bls12381r$2)) curve = await buildBls12381(singleThread, plugins);
...
var utils = _utils;
//#endregion
//#region node_modules/snarkjs/build/browser.esm.js
var fs = {};
async function open(fileName, openFlags, cacheSize, pageSize) {
	...
	return new FastFile(fd, await fd.stat(), cacheSize, pageSize, fileName);
}
var FastFile = class {
	constructor(fd, stats, cacheSize, pageSize, fileName) {
		...
		this.fd = fd;
		this.pos = 0;
		this.pageSize = pageSize || 256;
		while (this.pageSize < stats.blksize) this.pageSize *= 2;
	...
	__statusPage(s, p) {
		...
		logEntry.push("==" + s + " " + p);
		let S = "";
		for (let i = 0; i < self.pendingLoads.length; i++) if (self.pendingLoads[i].page == p) S = S + " " + i;
	...
	__printHistory(p) {
		...
		for (let i = 0; i < self.history[p].length; i++) for (let j = 0; j < self.history[p][i].length; j++) console.log("-> " + self.history[p][i][j]);
	}
	_triggerLoad() {
		const self = this;
		...
		while (self.pendingLoads.length > 0 && (typeof self.pages[self.pendingLoads[0].page] != "undefined" || freePages > 0 || deletablePages.length > 0)) {
			...
			if (typeof self.pages[load.page] != "undefined") {
				...
				if (self.pages[load.page].loading) self.pages[load.page].loading.push(load);
				else load.resolve();
				self.__statusPage("After Load (cached): ", load.page);
			} else {
				if (freePages) freePages--;
				else {
				...
				if (load.page >= self.totalPages) {
					...
					ops.push(self.fd.read(self.pages[load.page].buff, 0, self.pageSize, load.page * self.pageSize).then((res) => {
						...
						for (let i = 0; i < loading.length; i++) loading[i].resolve();
	...
	_triggerWrite() {
		...
		if (self.writing) Promise.all(ops).then(() => {
			self.writing = false;
	...
	_getDirtyPage() {
		for (let p in this.pages) if (this.pages[p].dirty) return p;
		return -1;
	}
	async write(buff, pos) {
		...
		while (r > 0) {
			...
			new Uint8Array(self.pages[p].buff.buffer, o, l).set(srcView);
			self.pages[p].dirty = true;
			self.pages[p].pendingOps--;
			self.pages[p].size = Math.max(o + l, self.pages[p].size);
			if (p >= self.totalPages) self.totalPages = p + 1;
			r = r - l;
			p++;
			o = 0;
	...
	async readToBuffer(buffDst, offset, len, pos) {
		if (len == 0) return;
		const self = this;
		if (len > self.pageSize * self.maxPagesLoaded * .8) {
	...
	_tryClose() {
		...
		if (self.error) self.pendingCloseReject(self.error);
		if (self._getDirtyPage() >= 0 || self.writing || self.reading || self.pendingLoads.length > 0) return;
		self.pendingClose();
	}
	close() {
		const self = this;
	...
	async writeULE32(v, pos) {
		const self = this;
		const tmpBuff32 = /* @__PURE__ */ new Uint8Array(4);
		new DataView(tmpBuff32.buffer).setUint32(0, v, true);
		await self.write(tmpBuff32, pos);
	}
	async writeUBE32(v, pos) {
		const self = this;
		const tmpBuff32 = /* @__PURE__ */ new Uint8Array(4);
		new DataView(tmpBuff32.buffer).setUint32(0, v, false);
		await self.write(tmpBuff32, pos);
	}
	async writeULE64(v, pos) {
		const self = this;
		const tmpBuff64 = /* @__PURE__ */ new Uint8Array(8);
...
var tmpBuff32$1 = /* @__PURE__ */ new Uint8Array(4);
var tmpBuff32v$1 = new DataView(tmpBuff32$1.buffer);
var tmpBuff64$1 = /* @__PURE__ */ new Uint8Array(8);
...
var MemFile = class {
	...
	close() {
		if (this.o.data.byteLength != this.totalSize) this.o.data = this.o.data.slice(0, this.totalSize);
	}
	async discard() {}
	async writeULE32(v, pos) {
		const self = this;
...
function createNew(o) {
	...
	fd.o.data = [];
	for (let i = 0; i < nPages - 1; i++) fd.o.data.push(new Uint8Array(PAGE_SIZE));
...
var tmpBuff32 = /* @__PURE__ */ new Uint8Array(4);
var tmpBuff32v = new DataView(tmpBuff32.buffer);
var tmpBuff64 = /* @__PURE__ */ new Uint8Array(8);
...
var BigMemFile = class {
	...
	async write(buff, pos) {
		...
		if (this.readOnly) throw new Error("Writing a read only file");
		this._resizeIfNeeded(pos + buff.byteLength);
		let p = Math.floor(pos / PAGE_SIZE);
		let o = pos % PAGE_SIZE;
		let r = buff.byteLength;
		while (r > 0) {
			const l = o + r > PAGE_SIZE ? PAGE_SIZE - o : r;
			const srcView = buff.slice(buff.byteLength - r, buff.byteLength - r + l);
			new Uint8Array(self.o.data[p].buffer, o, l).set(srcView);
			r = r - l;
			p++;
			o = 0;
		}
	...
	async readString(pos) {
		...
		while (!endOfStringFound) {
			...
			if (self.o.data[currentPage] === void 0) throw new Error("ERROR");
...
var O_RDONLY = 0;
var DEFAULT_CACHE_SIZE = 65536;
var DEFAULT_PAGE_SIZE = 8192;
async function createOverride(o, b, c) {
	...
	if (o.type == "file") return await open(o.fileName, 1538, o.cacheSize, o.pageSize);
	else if (o.type == "mem") return createNew$1(o);
	else if (o.type == "bigMem") return createNew(o);
	else throw new Error("Invalid FastFile type: " + o.type);
}
async function readExisting(o, b, c) {
	...
	if (typeof o === "string") o = {
		type: "mem",
		data: await fetch(o).then(function(res) {
			return res.arrayBuffer();
		}).then(function(ab) {
			return new Uint8Array(ab);
		})
	};
	if (o.type == "file") return await open(o.fileName, O_RDONLY, o.cacheSize, o.pageSize);
	else if (o.type == "mem") return await readExisting$2(o);
	else if (o.type == "bigMem") return await readExisting$1(o);
	else throw new Error("Invalid FastFile type: " + o.type);
}
...
async function readSection(fd, sections, idSection, offset, length) {
	...
	let buff;
	if (length < 1 << 30) buff = new Uint8Array(length);
	else buff = new BigBuffer(length);
...
var bls12381q = Scalar.e("1a0111ea397fe69a4b1ba7b6434bacd764774b84f38512bf6730d2a0f6b0f6241eabfffeb153ffffb9feffffffffaaab", 16);
var bn128q = Scalar.e("21888242871839275222246405745257275088696311157297823662689037894645226208583");
async function getCurveFromR(r, options) {
	let curve;
	let singleThread = options && options.singleThread;
	if (Scalar.eq(r, bn128r$1)) curve = await buildBn128(singleThread);
	else if (Scalar.eq(r, bls12381r$1)) curve = await buildBls12381(singleThread);
	else throw new Error(`Curve not supported: ${Scalar.toString(r)}`);
	return curve;
}
async function getCurveFromQ(q, options) {
	let curve;
	let singleThread = options && options.singleThread;
	if (Scalar.eq(q, bn128q)) curve = await buildBn128(singleThread);
	else if (Scalar.eq(q, bls12381q)) curve = await buildBls12381(singleThread);
	else throw new Error(`Curve not supported: ${Scalar.toString(q)}`);
	return curve;
}
async function getCurveFromName(name, options) {
	...
	if ([
		...
		"BN254",
		"ALTBN128"
	].indexOf(normName) >= 0) curve = await buildBn128(singleThread);
	else if (["BLS12381"].indexOf(normName) >= 0) curve = await buildBls12381(singleThread);
...
function G1b(a, b, c, d, msg, x) {
	...
	({Dh, Dl} = {
		Dh: u64$1.rotr32H(Dh, Dl),
		Dl: u64$1.rotr32L(Dh, Dl)
	});
	...
	({Bh, Bl} = {
		Bh: u64$1.rotrSH(Bh, Bl, 24),
		Bl: u64$1.rotrSL(Bh, Bl, 24)
	});
...
function G2b(a, b, c, d, msg, x) {
	...
	({Dh, Dl} = {
		Dh: u64$1.rotrSH(Dh, Dl, 16),
		Dl: u64$1.rotrSL(Dh, Dl, 16)
	});
	({h: Ch, l: Cl} = u64$1.add(Ch, Cl, Dh, Dl));
	({Bh, Bl} = {
		Bh: Bh ^ Ch,
	...
	({Bh, Bl} = {
		Bh: u64$1.rotrBH(Bh, Bl, 63),
		Bl: u64$1.rotrBL(Bh, Bl, 63)
	});
	BBUF[2 * a] = Al, BBUF[2 * a + 1] = Ah;
	BBUF[2 * b] = Bl, BBUF[2 * b + 1] = Bh;
	BBUF[2 * c] = Cl, BBUF[2 * c + 1] = Ch;
...
async function readHeader$1(fd, sections, toObject, options) {
	...
	if (protocolId === GROTH16_PROTOCOL_ID) return await readHeaderGroth16(fd, sections, toObject, options);
	else if (protocolId === PLONK_PROTOCOL_ID) return await readHeaderPlonk(fd, sections, toObject, options);
	else if (protocolId === FFLONK_PROTOCOL_ID) return await readHeaderFFlonk(fd, sections, toObject, options);
...
async function write(fd, witness, prime) {
	...
	await endWriteSection(fd);
	await startWriteSection(fd, 2);
	for (let i = 0; i < witness.length; i++) await writeBigInt(fd, witness[i], n8);
	await endWriteSection(fd);
}
...
async function joinABC(curve, zkey, a, b, c, logger) {
	...
	let outBuff;
	if (a instanceof BigBuffer) outBuff = new BigBuffer(a.byteLength);
			...
			runtime: {
				...
				showSharedRWMemory: function() {
					...
					if (majorVersion >= 2 && (minorVersion >= 1 || patchVersion >= 7)) {
						...
						msgStr += msg;
					} else console.log(Scalar.fromArray(arr, 4294967296));
				},
...
var WitnessCalculatorCircom1 = class {
	...
	setFr(p, v) {
		...
		if (self.Fr.geq(v, minShort) && self.Fr.leq(v, maxShort)) {
			let a;
			if (self.Fr.geq(v, self.Fr.zero)) a = Scalar.toNumber(v);
			else {
...
async function readPTauHeader(fd, sections) {
	...
	const curve = await getCurveFromQ(Scalar.fromRprLE(buff));
...
var _BigArray$1 = class {
	constructor(initSize) {
		...
		for (let i = 0; i < initSize; i += SUBARRAY_SIZE$1) this.arr[i / SUBARRAY_SIZE$1] = new Array(Math.min(SUBARRAY_SIZE$1, initSize - i));
		return this;
	}
	push() {
	...
	setElement(idx, value) {
		idx = parseInt(idx);
		const idx1 = Math.floor(idx / SUBARRAY_SIZE$1);
		if (!this.arr[idx1]) this.arr[idx1] = new Array(SUBARRAY_SIZE$1);
		const idx2 = idx % SUBARRAY_SIZE$1;
	...
	getKeys() {
		...
		for (let i = 0; i < this.arr.length; i++) if (this.arr[i]) {
			for (let j = 0; j < this.arr[i].length; j++) if (typeof this.arr[i][j] !== "undefined") newA.push(i * SUBARRAY_SIZE$1 + j);
		}
		return newA;
	}
};
var BigArray$2 = class {
	constructor(initSize) {
		...
		return new Proxy(obj, BigArrayHandler$1);
	}
};
...
async function readConstraints(fd, sections, r1cs, logger, loggerCtx) {
	...
	let bR1csPos = 0;
	let constraints;
	if (r1cs.nConstraints > 1 << 20) constraints = new BigArray$3();
	else constraints = [];
...
async function readMap(fd, sections, r1cs, logger, loggerCtx) {
	...
	let bMapPos = 0;
	let map;
	if (r1cs.nVars > 1 << 20) map = new BigArray$3();
	else map = [];
...
async function readCustomGatesUsesSection(fd, sections, options) {
	...
	let bR1csPos = 1;
	let customGatesUses;
	if (nCustomGateUses > 1 << 20) customGatesUses = new BigArray$3();
...
var bn128r = Scalar.e("21888242871839275222246405745257275088548364400416034343698204186575808495617");
async function r1csInfo(r1csName, logger) {
	...
	} else if (logger) logger.info(`Unknown Curve. Prime: ${Scalar.toString(cir.prime)}`);
...
async function wtnsCheck(r1csFilename, wtnsFilename, logger) {
	...
	if (!Scalar.eq(r1cs.prime, wtnsHeader.q)) throw new Error("Curve of the witness does not match the curve of the proving key");
...
async function phase2verifyFromInit(initFileName, pTauFileName, zkeyFileName, logger) {
	...
	if (!Scalar.eq(zkeyInit.q, zkey.q) || !Scalar.eq(zkeyInit.r, zkey.r) || zkeyInit.n8q != zkey.n8q || zkeyInit.n8r != zkey.n8r) {
...
async function zkeyExportVerificationKey(zkeyName, logger) {
	...
	else if (zkey.protocolId && zkey.protocolId === FFLONK_PROTOCOL_ID) res = await exportFFlonkVk(zkey);
...
var Polynomial = class Polynomial {
	...
	static fromCoefficientsArray(array, curve, logger) {
		const Fr = curve.Fr;
		let buff = array.length > 32768 ? new BigBuffer(array.length * Fr.n8) : new Uint8Array(array.length * Fr.n8);
	...
	static fromPolynomial(polynomial, curve, logger) {
		...
		let Fr = curve.Fr;
		let buff = length > 32768 ? new BigBuffer(length * Fr.n8) : new Uint8Array(length * Fr.n8);
	...
	blindCoefficients(blindingFactors) {
		...
		const blindedCoefficients = this.length() + blindingFactors.length > 32768 ? new BigBuffer((this.length() + blindingFactors.length) * this.Fr.n8) : new Uint8Array((this.length() + blindingFactors.length) * this.Fr.n8);
	...
	static async to4T(buffer, domainSize, blindingFactors, Fr) {
		...
		const a4 = domainSize * 4 > 32768 ? new BigBuffer(domainSize * 4 * Fr.n8) : new Uint8Array(domainSize * 4 * Fr.n8);
		a4.set(a, 0);
		const A4 = await Fr.fft(a4);
		if (blindingFactors.length === 0) return [a, A4];
		const a1 = domainSize + blindingFactors.length > 32768 ? new BigBuffer((domainSize + blindingFactors.length) * Fr.n8) : new Uint8Array((domainSize + blindingFactors.length) * Fr.n8);
		a1.set(a, 0);
		for (let i = 0; i < blindingFactors.length; i++) {
	...
	byXSubValue(value) {
		...
		let pol = new Polynomial(length > 32768 ? new BigBuffer(length * Fr.n8) : new Uint8Array(length * Fr.n8), this.curve, this.logger);
	...
	divBy(polynomial) {
		...
		this.coef = this.length() > 32768 ? new BigBuffer(this.length() * Fr.n8) : new Uint8Array(this.length() * Fr.n8);
	...
	divByMonic(m, beta) {
		const Fr = this.Fr;
		let d = this.degree();
		let quotient = new Polynomial(this.length() > 32768 ? new BigBuffer(this.length() * Fr.n8) : new Uint8Array(this.length() * Fr.n8), this.curve, this.logger);
		let bArr = [];
		for (let i = 0; i < m; i++) {
	...
	fastDivByVanishing(data) {
		...
		for (let i = 0; i < data.length; i++) {
			...
			let polTmp = new Polynomial(this.length() > 32768 ? new BigBuffer(this.length() * Fr.n8) : new Uint8Array(this.length() * Fr.n8), this.curve, this.logger);
			let ptr = this.coef;
	...
	divByXSubValue(value) {
		const coefs = this.length() > 32768 ? new BigBuffer(this.length() * this.Fr.n8) : new Uint8Array(this.length() * this.Fr.n8);
	...
	byX() {
		const coefs = this.length() + 1 > 32768 ? new BigBuffer(this.coef.byteLength + this.Fr.n8) : new Uint8Array(this.coef.byteLength + this.Fr.n8);
	...
	static async expX(polynomial, n, truncate = false) {
		...
		else if (1 === n) return await Polynomial.fromEvaluations(polynomial.coef, curve, polynomial.logger);
		const length = truncate ? polynomial.degree() : polynomial.length() - 1;
		const bufferDst = length * n + 1 > 32768 ? new BigBuffer((length * n + 1) * Fr.n8) : new Uint8Array((length * n + 1) * Fr.n8);
	...
	split(numPols, degPols, blindingFactors) {
		...
		for (let i = 0; i < numPols; i++) {
			...
			res[i] = new Polynomial(byteLength / this.Fr.n8 > 32768 ? new BigBuffer(byteLength) : new Uint8Array(byteLength), this.curve, this.logger);
	...
	truncate() {
		...
		if (deg + 1 < this.coef.byteLength / this.Fr.n8) {
			const newCoefs = deg + 1 > 32768 ? new BigBuffer((deg + 1) * this.Fr.n8) : new Uint8Array((deg + 1) * this.Fr.n8);
		...
		function computeLagrangePolynomial(i) {
			...
			for (let j = 0; j < xArr.length; j++) {
				...
				if (polynomial === void 0) {
					polynomial = new Polynomial(xArr.length > 32768 ? new BigBuffer(xArr.length * Fr.n8) : new Uint8Array(xArr.length * Fr.n8), curve);
	...
	static zerofierPolynomial(xArr, curve) {
		const Fr = curve.Fr;
		let polynomial = new Polynomial(xArr.length + 1 > 32768 ? new BigBuffer((xArr.length + 1) * Fr.n8) : new Uint8Array((xArr.length + 1) * Fr.n8), curve);
...
async function plonk16Prove(zkeyFileName, witnessFileName, logger, options) {
	...
	async function computeT() {
		...
		for (let i = 0; i < zkey.nPublic; i++) await fdZKey.readToBuffer(evaluations.Lagrange.eval, i * sDomain * 4, sDomain * 4, zkeySections[ZKEY_PL_LAGRANGE_SECTION][0].p + i * 5 * sDomain + sDomain);
...
async function fflonkProve(zkeyFileName, witnessFileName, logger, options) {
	...
	const { fd: fdZKey, sections: zkeySections } = await readBinFile(zkeyFileName, "zkey", 2);
	const zkey = await readHeader$1(fdZKey, zkeySections, void 0, options);
	if (zkey.protocolId !== FFLONK_PROTOCOL_ID) throw new Error("zkey file is not fflonk");
	if (!Scalar.eq(zkey.r, wtns.q)) throw new Error("Curve of the witness does not match the curve of the proving key");
	if (wtns.nWitness !== zkey.nVars - zkey.nAdditions) throw new Error(`Invalid witness length. Circuit: ${zkey.nVars}, witness: ${wtns.nWitness}, ${zkey.nAdditions}`);
	const curve = zkey.curve;
	const Fr = curve.Fr;
	...
	async function round1() {
		...
		async function computeT0() {
			...
			evaluations.lagrange1 = new Evaluations(await readSection(fdZKey, zkeySections, ZKEY_FF_LAGRANGE_SECTION), curve, logger);
