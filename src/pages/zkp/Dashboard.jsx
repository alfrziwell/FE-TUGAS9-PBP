...
const Dashboard = () => {
  ...
  useEffect(() => {
    // Pengecekan sesi: Jika tidak ada username di localStorage, redirect ke login
    ...
    const fetchProfile = async () => {
      try {
        // Menggunakan BrowserProvider read-only untuk memanggil fungsi view
        if (!window.ethereum) {
        ...
        // Memanggil fungsi getUserProfile dari smart contract
        const [fullName, username] = await contract.getUserProfile(storedUsername);

        setUser({ fullName, username });
      } catch (err) {
        console.error(err);
        setError('Gagal memuat profil dari blockchain. Pastikan MetaMask terhubung ke jaringan Sepolia.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-primary font-poppins text-secondary flex flex-col justify-center items-center">
        ...
        <p className="text-sm text-secondary/60">Memuat profil dari blockchain...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-primary font-poppins text-secondary flex flex-col justify-between selection:bg-accent selection:text-secondary overflow-hidden relative">
      ...
      <main className="grow max-w-6xl mx-auto px-6 py-8 flex flex-col justify-center items-center relative z-10 w-full">
        ...
        {/* Error */}
        {error && (
          <div className="w-full max-w-2xl mb-6 p-4 bg-red-50 border-l-4 border-red-500 rounded-r-lg text-xs text-red-700">
            {error}
          </div>
        )}
        ...
        <div className="w-full max-w-2xl bg-white/70 border border-secondary/10 backdrop-blur-md p-6 rounded-2xl shadow-sm mb-6 text-center sm:text-left flex flex-col sm:flex-row items-center sm:space-x-5">
          ...
          <div>
            ...
            <p className="text-sm text-secondary/60 font-light">
              Anda berhasil masuk dengan verifikasi Zero-Knowledge Proof di blockchain Sepolia.
            </p>
          </div>
        </div>
        ...
        <div className="w-full max-w-2xl grid grid-cols-1 gap-6">
          ...
          <div className="p-6 rounded-2xl bg-white/70 border border-secondary/10 backdrop-blur-md shadow-sm">
            <h3 className="text-lg font-bold text-secondary mb-4 flex items-center space-x-2">
              ...
              <span>Detail Akun On-Chain</span>
            </h3>
            ...
            <div className="divide-y divide-secondary/10 text-sm">
              ...
              <div className="py-2.5 flex justify-between">
                <span className="text-secondary/60 font-light">Jaringan</span>
                <span className="font-semibold px-2 py-0.5 rounded-full bg-accent text-xs">
                  Sepolia Testnet
                </span>
              </div>
              <div className="py-2.5 flex justify-between items-center">
                <span className="text-secondary/60 font-light">Smart Contract</span>
                <a
                  href={`https://sepolia.etherscan.io/address/${CONTRACT_ADDRESS}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="font-semibold text-xs text-secondary hover:underline truncate max-w-[200px]"
                >
